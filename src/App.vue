...
import { useSherpaTts } from './composables/useSherpaTts'
...
const SETTINGS_STORAGE_KEY = 'digital-listen:settings:v2'
...
const selectedRatePreset = ref<SpeechRatePreset>(
  asSpeechRatePreset(persistedSettings.speechRatePreset) ?? 'normal',
)
const moduleError = ref('')

const { engineMessage, engineStatus, speaking, speak, stop } = useSherpaTts()
...
const engineStatusText = computed(() => {
  if (engineStatus.value === 'loading') {
    return engineMessage.value || engineStatusLabels.loading
  }
  return engineStatusLabels[engineStatus.value]
})
...
const speakCurrentItem = () => {
  ...
  void speak(currentItem.value.speakText, selectedRatePreset.value)
}
    ...
    <section class="panel">
      ...
      <div class="panel-group">
        ...
        <div class="settings-grid">
          ...
          <div class="field">
            <span>语音引擎</span>
            <span :class="['engine-status', `engine-status-${engineStatus}`]">
              本地开源引擎（sherpa-onnx）· {{ engineStatusText }}
            </span>
          </div>
        </div>
        <p v-if="engineStatus === 'error'" class="error-text">{{ engineMessage }}</p>
      </div>

      <div class="panel-group">
        <h2 class="group-title">题目设置</h2>
        <div class="settings-grid">
          <template v-if="activeModule === 'phone'">
            <label class="field">
              <span>电话号码位数（3 - 20）</span>
        ...
        <p v-else class="hint-text">修改设置后，点击下方“重新开始”或按快捷键 R 生效。</p>
      </div>

      <div class="panel-group">
        <h2 class="group-title">练习控制</h2>
        <div class="actions">
          <button type="button" class="primary" @click="restartSession">重新开始（R）</button>
          <button type="button" :disabled="!currentItem || engineStatus === 'error'" @click="repeatCurrent">
            重复发音（空格）
          </button>
        ...
        <div class="status">
          ...
          <span>快捷键：空格重复，右箭头下一题，R 重新开始</span>
        </div>
