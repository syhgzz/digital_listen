...
<template>
  <main class="app">
    ...
    <div v-if="engineLoading" class="engine-loading">
      <div class="engine-loading-card">
        <p class="engine-loading-title">正在加载统一语音引擎</p>
        <p class="engine-loading-desc">首次使用需下载语音模型（约 80MB），请耐心等待…</p>
        <div class="engine-loading-bar">
          <div
            class="engine-loading-fill"
            :style="{ width: engineProgress.percent + '%' }"
          ></div>
        </div>
        <p class="engine-loading-pct">{{ engineProgress.percent }}%</p>
        <p class="engine-loading-msg">{{ engineProgress.message }}</p>
      </div>
    </div>
    ...
    <section class="panel">
      ...
      <!-- Error messages -->
      <p v-if="!anyEngineConfigured && !engineLoading" class="error-text">当前没有可用语音引擎，请确保浏览器支持 Web Speech API 或 WASM。</p>
      ...
      <div class="actions">
        ...
        <button type="button" :disabled="!currentItem || !anyEngineConfigured || engineLoading" @click="repeatCurrent">
          重复发音（空格）
        </button>
        <button type="button" :disabled="!hasNextItem" @click="nextItem">下一题（→）</button>
      </div>

      <!-- Status bar -->
      <div class="status">
        <span>进度：{{ progressLabel }}</span>
        <span v-if="speaking">朗读中…</span>
        <span v-if="currentItem">快捷键：空格重复 · → 下一题 · R 重新开始</span>
      </div>
